...
mod ed25519_impls {
	...
	impl Readable for ed25519::PublicKey {
		fn take_from(b: &mut Reader<'_>) -> Result<Self> {
			let bytes = b.take(32)?;
			Self::from_bytes(array_ref![bytes, 0, 32]).map_err(|_| {
				ErrorKind::Tor("Couldn't decode Ed25519 public key".to_string()).into()
			})
		}
	}
	...
	impl Readable for ed25519::Signature {
		fn take_from(b: &mut Reader<'_>) -> Result<Self> {
			let bytes = b.take(64)?;
			Self::from_bytes(array_ref![bytes, 0, 64]).map_err(|_| {
				ErrorKind::Tor("Couldn't decode Ed25519 signature.".to_string()).into()
			})
		}
	}
}
