...
pub trait Readable: Sized {
	...
	/// operations.  If you have an object that must
	/// not be used until it is finally validated, consider
	/// making this function return a wrapped type that can be
	/// unwrapped later on once it gets checked.
...
/// the `extract` and `_extract_n` methods will panic if the underlying
...
///
/// ```
/// use nioruntime_tor::reader::Reader;
...
/// ```
/// use nioruntime_tor::reader::{Reader,Readable};
/// use std::net::Ipv4Addr;
/// let msg = [ 0x00, 0x04, 0x7f, 0x00, 0x00, 0x01];
/// let mut r = Reader::from_slice(&msg[..]);
///
/// let tp: u16 = r.extract().unwrap();
/// assert_eq!(tp, 4);
///
/// ```
pub struct Reader<'a> {
...
impl<'a> Reader<'a> {
	...
	pub fn advance(&mut self, n: usize) -> Result<()> {
		if n > self.remaining() {
			return Err(ErrorKind::Tor("Truncated".to_string()).into());
		}
		self.off += n;
		Ok(())
	}
	...
	pub fn should_be_exhausted(&self) -> Result<()> {
		if self.remaining() != 0 {
			return Err(ErrorKind::Tor("ExtraneousBytes".to_string()).into());
		}
		Ok(())
	}
	...
	/// to begin with.
	pub fn _truncate(&mut self, n: usize) {
		if n < self.remaining() {
	...
	/// assert_eq!(r.take(5).unwrap(), b"World");
	/// r.should_be_exhausted().unwrap();
	///
	/// ```
	...
	/// let mut v2 = vec![0; 5];
	/// let mut r = Reader::from_slice(m);
	/// r.take_into(&mut v1[..]).unwrap();
	/// assert_eq!(r.take_u8().unwrap(), b' ');
	/// r.take_into(&mut v2[..]).unwrap();
	...
	/// let m = b"Hello\0wrld";
	/// let mut r = Reader::from_slice(m);
	/// assert_eq!(r._take_until(0).unwrap(), b"Hello");
	/// assert_eq!(r._into_rest(), b"wrld");
	///
	/// ```
	pub fn _take_until(&mut self, term: u8) -> Result<&'a [u8]> {
		let pos = self.b[self.off..].iter().position(|b| *b == term).ok_or({
			let error: Error = ErrorKind::Tor("Truncated".to_string()).into();
			error
		})?;
	...
	///
	/// On failure, consumes nothing.
	pub fn _extract_n<E: Readable>(&mut self, n: usize) -> Result<Vec<E>> {
...
mod tests {
	...
	fn bytecursor_read_ok() {
		...
		assert_eq!(bc._total_len(), 43); // no change
	...
	fn read_u128() {
		...
		let mut r = Reader::_from_bytes(&bytes);
	...
	fn bytecursor_read_missing() {
		...
		assert_eq!(bc._total_len(), 7);

		assert_eq!(bc.take_u64().is_err(), true);
		assert_eq!(bc.take(8).is_err(), true);
		assert_eq!(bc.peek(8).is_err(), true);
		...
		assert_eq!(bc.take_u32().is_err(), true);
		...
		assert_eq!(bc.take_u16().is_err(), true);
		...
		assert_eq!(bc.take_u8().is_err(), true);
		assert_eq!(bc._total_len(), 7);
	}
	...
	fn advance_too_far() {
		...
		assert_eq!(r.advance(6).is_err(), true);
		assert_eq!(r.remaining(), 5);
		assert_eq!(r.advance(5).is_ok(), true);
	...
	fn truncate() {
		...
		assert_eq!(r.consumed(), 5);
		assert_eq!(r.take_u8().unwrap(), 0x20);
		assert_eq!(r._into_rest(), &b"universe"[..]);
	}
	...
	fn exhaust() {
		...
		assert_eq!(r.should_be_exhausted().is_err(), true);
		r.take(4).unwrap();
		assert_eq!(r.should_be_exhausted().is_err(), true);
		r.take(1).unwrap();
		assert_eq!(r.should_be_exhausted().is_ok(), true);
	...
	fn take_until() {
		...
		assert_eq!(r._take_until(b' ').unwrap(), &b"si"[..]);
		assert_eq!(r._take_until(b' ').unwrap(), &b"vales"[..]);
		assert_eq!(r._take_until(b' ').is_err(), true);
	}
	...
	fn extract() {
		...
		let les: Vec<LenEnc> = r._extract_n(4).unwrap();
		...
		assert_eq!(
			le.unwrap_err().kind(),
			ErrorKind::Tor("Truncated".to_string())
		);
		assert_eq!(r.remaining(), 1);
		...
		let les: Result<Vec<LenEnc>> = r._extract_n(10);
		assert_eq!(
			les.unwrap_err().kind(),
			ErrorKind::Tor("Truncated".to_string())
		);
