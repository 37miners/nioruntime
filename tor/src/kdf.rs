...
use nioruntime_deps::digest::{ExtendableOutput, Update, XofReader};
use nioruntime_deps::sha1::Sha1;
use nioruntime_deps::sha2::Sha256;
use nioruntime_deps::sha3::Shake256;
...
pub(crate) trait Kdf {
	...
	fn derive(&self, seed: &[u8], n_bytes: usize) -> Result<SecretBytes, Error>;
}
...
impl Kdf for LegacyKdf {
	fn derive(&self, seed: &[u8], n_bytes: usize) -> Result<SecretBytes, Error> {
		...
		if n_bytes > Sha1::output_size() * (256 - (k as usize)) {
			return Err(ErrorKind::Tor("InvalidOutputLength".into()).into());
		}
...
impl Kdf for Ntor1Kdf<'_, '_> {
	fn derive(&self, seed: &[u8], n_bytes: usize) -> Result<SecretBytes, Error> {
		let hkdf = nioruntime_deps::hkdf::Hkdf::<Sha256>::new(Some(self.t_key), seed);

		let mut result = Zeroizing::new(vec![0; n_bytes]);
		hkdf.expand(self.m_expand, &mut result[..]).map_err(|_| {
			let error: Error = ErrorKind::Tor("InvalidOutputLength".into()).into();
			error
		})?;
		Ok(result)
	}
...
mod test {
	...
	fn clearbox_tap_kdf() {
		...
		let result = LegacyKdf::_new(6).derive(input, 99).unwrap();
	...
	fn testvec_tap_kdf() {
		...
		fn expand(b: &[u8]) -> SecretBytes {
			LegacyKdf::_new(0).derive(b, 100).unwrap()
		}
	...
	#[test]
	fn fail_tap_kdf() {
		let result = LegacyKdf::_new(6).derive(&b"x"[..], 10000);
	...
	fn clearbox_ntor1_kdf() {
		...
		let kdf = nioruntime_deps::hkdf::Hkdf::<Sha256>::new(Some(&b"key"[..]), &input[..]);
