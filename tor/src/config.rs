...
impl TorRcConfig {
	...
	pub fn write_to_file(&self, file_path: &str) -> Result<(), Error> {
		let mut file = File::create(file_path).map_err(|e| {
			ErrorKind::IOError(format!("Unable to create file {}, {}", file_path, e))
		})?;
		for item in &self.items {
			file.write_all(item.name.as_bytes()).map_err(|e| {
				ErrorKind::IOError(format!("Unable to write into file {}, {}", file_path, e))
			})?;
...
pub fn create_onion_service_sec_key_file(
	...
	let mut file = File::create(key_file_path).map_err(|e| {
		ErrorKind::IOError(format!("Unable to create file {}, {}", key_file_path, e))
	})?;
...
pub fn create_onion_service_hostname_file(os_directory: &str, hostname: &str) -> Result<(), Error> {
	...
	let mut file = File::create(file_path)
		.map_err(|e| ErrorKind::IOError(format!("Unable to create file {}, {}", file_path, e)))?;
	file.write_all(&format!("{}.onion\n", hostname).as_bytes())
		.map_err(|e| {
			ErrorKind::IOError(format!("Fail to store data to file {}, {}", file_path, e))
		})?;
	Ok(())
}
...
pub fn create_onion_auth_clients_dir(os_directory: &str) -> Result<(), Error> {
	...
	fs::create_dir_all(auth_dir_path).map_err(|e| {
		ErrorKind::IOError(format!("Unable to create dir {}, {}", auth_dir_path, e))
	})?;
	Ok(())
}
...
pub fn output_onion_service_config(
	...
	fs::create_dir_all(&hs_dir_file_path).map_err(|e| {
		ErrorKind::IOError(format!("Unable to create dir {}, {}", hs_dir_file_path, e))
	})?;

	create_onion_service_sec_key_file(&hs_dir_file_path, &d_sec_key)?;
	create_onion_service_pub_key_file(
		&hs_dir_file_path,
		&address.to_ed25519().map_err(|e| {
			let error: Error =
				ErrorKind::InternalError(format!("v3Onion from pub key err: {}", e)).into();
			error
		})?,
	)?;
...
pub fn output_tor_listener_config(
	...
	fs::create_dir_all(&tor_data_dir)
		.map_err(|e| ErrorKind::IOError(format!("Unable to create dir {}, {}", tor_data_dir, e)))?;
...
pub fn _output_tor_sender_config(
	...
	// create data directory if it doesn't exist
	fs::create_dir_all(&tor_config_dir).map_err(|e| {
		ErrorKind::IOError(format!("Unable to create dir {}, {}", tor_config_dir, e))
	})?;
