...
pub fn client_send_intro<R>(
	rng: &mut R,
	proto_input: &HsNtorClientInput,
) -> Result<(HsNtorClientState, Vec<u8>), Error>
where
	R: RngCore + CryptoRng,
{
...
pub fn client_receive_rend<T>(
	state: &HsNtorClientState,
	msg: T,
) -> Result<HsNtorHkdfKeyGenerator, Error>
where
	T: AsRef<[u8]>,
{
...
pub fn server_receive_intro<R, T>(
	...
	msg: T,
) -> Result<(HsNtorHkdfKeyGenerator, Vec<u8>, Vec<u8>), Error>
where
	R: RngCore + CryptoRng,
	T: AsRef<[u8]>,
{
...
fn hs_ntor_mac(key: &[u8], message: &[u8]) -> Result<MacTag, Error> {
	let k_len = key.len();
