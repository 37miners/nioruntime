...
//!
//! ```
//! use nioruntime_deps::base64::decode;
...
//! // Decode the base64.
//! let cert_bin = nioruntime_deps::base64::decode(cert_base64).unwrap();
//!
//! // Decode the cert and check its signature.
//! let cert = rust_tor::Ed25519Cert::decode(&cert_bin).unwrap()
		...
		Ok(match ext_type {
			ExtType::SIGNED_WITH_ED25519_KEY => {
				...
				CertExt::SignedWithEd25519(SignedWithEd25519Ext {
					pk: ed25519::PublicKey::from_bytes(body).map_err(|_| {
						let error: Error =
...
impl KeyUnknownCert {
	...
	pub fn check_key(self, pkey: &Option<ed25519::PublicKey>) -> Result<UncheckedCert, Error> {
...
impl UncheckedCert {
	...
	pub fn dangerously_split(
		self,
	) -> Result<(SigCheckedCert, ed25519::ValidatableEd25519Signature), Error> {
	...
	pub fn _peek_signing_key(&self) -> &ed25519::PublicKey {
		self.cert
			.signed_with
			.as_ref()
...
mod test {
	...
	#[test]
	fn parse_unrecognized_ext() -> Result<(), Error> {
		...
		let mut r = Reader::from_slice(&b);
		let e: Result<CertExt, Error> = r.extract();
		assert!(e.is_err());
		assert_eq!(
			e.err().unwrap().kind(),
			ErrorKind::Tor(
