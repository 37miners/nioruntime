...
#[derive(Clone)]
pub(crate) struct RelayCellBody(pub RawCellBody);
...
pub(crate) trait CryptInit: Sized {
	...
	fn construct<K: crate::crypto::handshake::KeyGenerator>(keygen: K) -> Result<Self, Error> {
...
impl InboundClientCrypt {
	...
	pub(crate) fn decrypt(&mut self, cell: &mut RelayCellBody) -> Result<(HopNum, &[u8]), Error> {
...
mod test {
	...
	use nioruntime_deps::rand::RngCore;
