...
use nioruntime_deps::rand::{CryptoRng, RngCore};
...
impl super::ClientHandshake for CreateFastClient {
	...
	fn client2<T: AsRef<[u8]>>(state: Self::StateType, msg: T) -> Result<Self::KeyGen, Error> {
		...
		if msg.len() != FAST_S_HANDSHAKE_LEN {
			return Err(ErrorKind::Tor("BadCircHandshake".into()).into());
		}
		let mut inp = Vec::new();
		inp.extend(&state.0[..]);
		inp.extend(&msg[0..20]);
...
impl super::ServerHandshake for CreateFastServer {
	...
	fn server<R: RngCore + CryptoRng, T: AsRef<[u8]>>(
		...
		if msg.len() != FAST_C_HANDSHAKE_LEN {
			return Err(ErrorKind::Tor("Handhsake".into()).into());
		}
		...
		let kh = LegacyKdf::new(0).derive(&inp[..], 20).map_err(|e| {
			let error: Error = ErrorKind::Tor(format!("Can't expand key: {}", e)).into();
			error
		})?;
