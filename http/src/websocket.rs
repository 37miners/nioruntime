...
use nioruntime_deps::byteorder::{BigEndian, ByteOrder};
	                                                                         ...
	                                                                         //+ Unpin
{
	...
	let (messages, len) = build_messages(buffer)?;
...
mod test {
	...
	fn check_message(message: WebSocketMessage) -> Result<(), Error> {
		...
		let messages = build_messages(&mut data[..].to_vec())?;
		assert!(messages.0.len() == 1);

		assert_eq!(message, messages.0[0]);
		Ok(())
	}
